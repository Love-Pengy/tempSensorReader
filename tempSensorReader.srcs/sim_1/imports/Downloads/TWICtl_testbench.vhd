...
architecture behavior of TWI_testbench is
  ...
  signal CLK_sig, RESET_sig, START_sig, SRST_sig : std_logic := '0';					-- signals to connect to the TWI
  signal SCL_sig, MSG_I_sig, STB_I_sig, DONE_O_sig, ERR_O_sig, SDA_sig : std_logic; 
  signal A_I_sig, D_I_sig,  D_O_sig : STD_LOGIC_VECTOR (7 downto 0);
  signal startClk_sig : std_logic := '0';
  ...
  constant startPeriod : time := 2000 ms; -- 2 hz clock 
  ...
  begin
    ...
    process(startClk_sig)
        begin
            startCLK_sig <= not startClk_sig after startPeriod/2;
    end process; 
